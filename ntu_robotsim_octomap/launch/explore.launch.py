...
def generate_launch_description():
    pkg_ntu_robotsim_octomap = get_package_share_directory('ntu_robotsim_octomap')
    
    # Path to the explore_lite parameters
    explore_params = os.path.join(
        pkg_ntu_robotsim_octomap,
        'config',
        'explore_params.yaml'
    )
    ...
    launch_unified = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_ntu_robotsim_octomap, 'launch', 'unified.launch.py')
        )
    )
